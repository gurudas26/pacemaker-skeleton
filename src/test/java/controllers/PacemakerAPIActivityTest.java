...
public class PacemakerAPIActivityTest {
	  ...
	  public void testCreateActivity() {
		  ...
		  Activity act = new Activity("march", "ground", 1.0);
		  Activity useract = pacemaker.createActivity(user.id, "march", "ground", 1.0);
	  ...
	  public void testGetActivities() {
		  ...
		  assertEquals (0, user.activities.size());
		  pacemaker.createActivity(user.id, "march", "ground", 1.0);
		  pacemaker.createActivity(user.id, "run", "work", 2.0);
		  pacemaker.createActivity(user.id, "bike", "park", 3.0);
	  ...
	  public void testListActivitiesSorted() {
		  ...
		  assertEquals (0, user.activities.size());
		  pacemaker.createActivity(user.id, "walk", "park", 4.0);
		  pacemaker.createActivity(user.id, "march", "work", 5.0);
		  pacemaker.createActivity(user.id, "bike", "ground", 3.0);
		  
		  Activity act;
		  
		  act = pacemaker.listActivities(user.id, "type").get(0);
		  assertEquals("bike",act.type);
		  
		  act = pacemaker.listActivities(user.id, "location").get(0);
		  assertEquals("ground",act.location);
	  ...
	  public void testDeleteActivities() {
		  ...
		  pacemaker.createActivity(user.id, "run", "ground", 4.0);
		  pacemaker.createActivity(user.id, "march", "work", 5.0);
		  pacemaker.createActivity(user.id, "bike", "park", 3.0);
		  assertEquals (3,pacemaker.getActivities(user.id).size());
